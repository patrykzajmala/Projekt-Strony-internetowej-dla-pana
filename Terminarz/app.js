...
let currentDate = new Date(2026, 4, 22); 
let eventsData = [];
let selectedDateStr = ""; 
...
async function saveEvent(e) {
    ...
    const formattedInputDate = inputDateRaw.replace('T', ' '); 

    const isTimeTaken = eventsData.some(ev => ev.event_date === formattedInputDate);
    ...
    if (isTimeTaken) {
        errorContainer.textContent = "⚠️ Termin zajęty! Masz już zaplanowane zdarzenie na tę dokładną godzinę.";
        return;
    }
